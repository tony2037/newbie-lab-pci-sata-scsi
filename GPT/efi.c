 ...
 * If valid, returns pointers to newly allocated GPT header and PTEs.
 */
static int pgpt_recover(struct parsed_partitions *state, gpt_header **pgpt, gpt_entry **pptes, gpt_header **agpt, gpt_entry **aptes)
{
	u32 crc, origcrc;
	u64 lastlba;
	...
	if (!(*pgpt = alloc_read_gpt_header(state, GPT_PRIMARY_PARTITION_TABLE_LBA))) {
	 ...
	 * the GUID Partition Table */
	if (le64_to_cpu((*pgpt)->my_lba) != GPT_PRIMARY_PARTITION_TABLE_LBA) {
		pr_warn("[ZTEX][RECOVER]GPT my_lba incorrect: %lld != %lld\n",
			 (unsigned long long)le64_to_cpu((*pgpt)->my_lba),
			 (unsigned long long)GPT_PRIMARY_PARTITION_TABLE_LBA);
		(*pgpt)->my_lba = cpu_to_le64(GPT_PRIMARY_PARTITION_TABLE_LBA);
	}
	...
	pr_warn("[ZTEX][RECOVER]The right GUID Partitition Entry Array CRC: %x, and we have %x\n", 
